...
class TGN(torch.nn.Module):
    ...
    def forward(self, train_ngh_finder, batchsize, source_nodes, destination_nodes, edge_times, node_raw_features,
        ...
        self.historical_info = torch.from_numpy(np.zeros((len(source_nodes), 768 * 2))).to(self.device)
        ...
        for k in range(0, num_batch):
            ...
            # node_embedding.shape [batch_size, dim=768]
            ...
            for i in range(current_embedding.shape[0]):
                # print('ids:', (start_idx+i), i)
                self.historical_info[start_idx + i] = current_embedding[i]
