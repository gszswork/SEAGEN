...
from utils.dataset import loadTree, loadPHEMEData
...
parser.add_argument('--alpha', type=float, default=5e-5, help='The coeffecient of multi-task learning.')
...
def train_TGN(args, treeDic, x_test, x_train, weight_decay, patience, device):
    ...
    traindata_list, testdata_list = loadPHEMEData(x_train, x_test)
    print("len(traindata_list)", len(traindata_list))
    print("len(testdata_list)", len(testdata_list))

    writer = SummaryWriter('SEAGEN_PHEME/model'+str(args.alpha))
...
def evaluate(model, fold0_x_test, fold0_x_train, device):
    model = model.to(device)
    traindata_list, testdata_list = loadPHEMEData(fold0_x_train, fold0_x_test)
    num_item = 0
    ok = 0
...
if __name__ == '__main__':
    ...
    length = len(pheme_ids)

    fold0_x_test = np.array(pheme_ids[:int(0.2*length)])
    fold0_x_train = np.array(pheme_ids[int(0.2*length):])

    fold1_x_test = np.array(pheme_ids[int(0.2*length): int(0.4*length)])
    fold1_x_train = np.concatenate((np.array(pheme_ids[:int(0.2*length)]), np.array(pheme_ids[int(0.4*length):])))

    fold2_x_test = np.array(pheme_ids[int(0.4*length): int(0.6*length)])
    fold2_x_train = np.concatenate((pheme_ids[:int(0.4*length)], pheme_ids[int(0.6*length):]))

    fold3_x_test = np.array(pheme_ids[int(0.6*length): int(0.8*length)])
    fold3_x_train = np.concatenate((pheme_ids[:int(0.6*length)], pheme_ids[int(0.8*length):]))

    fold4_x_test = np.array(pheme_ids[int(0.8*length):])
    fold4_x_train = np.array(pheme_ids[:int(0.8*length)])
