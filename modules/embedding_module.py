...
class GraphEmbedding(EmbeddingModule):
  ...
  def compute_embedding(self, memory, source_nodes, timestamps, n_layers, n_neighbors=20, time_diffs=None,
    ...
    timestamps_torch = torch.unsqueeze(torch.from_numpy(timestamps).double().to(self.device), dim=1)
    source_nodes_time_embedding = self.time_encoder(torch.zeros_like(timestamps_torch)).float()
    ...
    if n_layers == 0:
      ...
      neighbors, edge_idxs, edge_times, edge_times_spans = self.neighbor_finder.get_temporal_neighbor(
        source_nodes,
        timestamps,
      ...
      edge_time_embeddings = self.time_encoder(edge_deltas_torch).float()
